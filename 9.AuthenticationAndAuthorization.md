...
| Session                                                                        | Cookie                                                                                 | Oauth 2                                                                                                                                                                 | JWT                                                                                                                                                                                                                                       | SSO                                                                                                                    |
| ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Session không được lưu trữ trên trình duyệt.                                   | Cookie được lưu trữ trên trình duyệt của người dùng.                                   | Oauth 2 sử dụng giao thức SSL  đảm bảo dữ liệu giữa máy chủ web và trình duyệt vẫn giữ được tính riêng tư để lưu token truy cập của người dùng.                         | Khi cần thực hiện việc bảo mật một thông tin<br>bất kỳ thì JWT chính là sự lựa chọn tốt nhất.                                                                                                                                             | Giảm số lượng tên đăng nhập (username) và mật khẩu (password) mà người dùng cần phải ghi nhớ.                          |
| Dữ liệu session được lưu trữ ở phía server.                                    | Dữ liệu cookie được lưu trữ ở phía client.                                             | Nó cho phép truy cập hạn chế vào dữ liệu của người dùng và cho phép truy cập khi authorization token hết hạn.                                                           | Nhỏ gọn, Phổ biến và dễ xử lý vì Bộ phân tích cú pháp JSON được sử dụng phổ biến trong hầu hết tất cả các ngôn ngữ lập trình, và được xử lý dễ dàng trên mọi thiết bị miễn là còn kết nối với mạng internet nhất là các thiết bị di động. | Giảm số lần phải nhập thông tin tên đăng nhập và mật khẩu.                                                             |
| Dữ liệu session không dễ dàng sửa đổi<br>vì chúng được lưu trữ ở phía máy chủ. | Dữ liệu cookie dễ dàng sửa đổi hoặc<br> đánh cắp khi chúng được lưu trữ ở phía client. | Khả năng chia sẻ dữ liệu cho người dùng<br>mà không phải tiết lộ thông tin cá nhân.                                                                                     | JWT là một phương thức khá tốt để<br>đảm bảo an toàn khi thực hiện việc truyền đạt hay trao đổi thông tin giữa các thành viên dựa trên signature của JWT.                                                                                 | Những rủi ro về việc lộ thông tin người dùng khi đăng nhập nhiều lần cũng được tiết chế lại.                           |
| Sau khi đóng trình duyệt sẽ hết phiên làm việc (session)                       | Dữ liệu cookie có sẵn trong trình duyệt đến khi expired.                               | Nếu ứng dụng/web của bạn được kết nối với trung tâm và tài khoản trung tâm bị hack, thì nó sẽ dẫn đến các ảnh hưởng nghiêm trọng trên một số trang web thay vì chỉ một. | Token chỉ dựa vào một khóa duy nhất vì vậy nếu vì bất cứ lý do gì mà khóa này bị mất hoặc bị lộ thì sẽ gây đến các hậu quả nghiêm trọng. Token có kích thước lớn và thời gian sử dụng ngắn hơn session.                                   | Phát sinh thêm các chi phí phát triển khi thông qua server bên thứ ba.<br>Phải phụ thuộc vào bên cung cấp dịch vụ SSO. |
